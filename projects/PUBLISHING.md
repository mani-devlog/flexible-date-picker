# Building and publishing @manidev/ngx-flexible-date-picker
...
| Item | Location |
|------|----------|
| Package name | `@manidev/ngx-flexible-date-picker` |
...
3. **npm account** with permission to publish `@manidev/ngx-flexible-date-picker` (or your chosen scoped name).
...
```bash
npm view @manidev/ngx-flexible-date-picker
```
...
```json
{
  "name": "@manidev/ngx-flexible-date-picker",
  "version": "0.0.1",
...
This package is published under the `@manidev` scope. Ensure the library `package.json` includes:
...
|--------------------------------------|---------|
| `fesm2022/manidev-ngx-flexible-date-picker.mjs` | ESM bundle |
| `fesm2022/manidev-ngx-flexible-date-picker.mjs.map` | Source map |
...
```css
@import '@manidev/ngx-flexible-date-picker/src/lib/themes/flexible-date-picker.css';
```
...
```bash
npm pack
tar -tzf manidev-ngx-flexible-date-picker-*.tgz
rm manidev-ngx-flexible-date-picker-*.tgz
```
...
3. Consumers install with:
   ...
   ```bash
   npm install @manidev/ngx-flexible-date-picker@beta
   ```
...
1. **Registry metadata:**
   ...
   ```bash
   npm view @manidev/ngx-flexible-date-picker
   npm view @manidev/ngx-flexible-date-picker version
   ```
...
4. **Dist-tags (optional):**
   ...
   ```bash
   npm dist-tag ls @manidev/ngx-flexible-date-picker
   ```
